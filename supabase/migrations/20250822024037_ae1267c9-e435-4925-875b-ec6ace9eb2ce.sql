...
-- 2. Update existing records to link them to the correct users
UPDATE public.professional_earnings pe
SET professional_user_id = pa.applicant_user_id
FROM public.professional_applications pa
WHERE pe.professional_id = pa.id
AND pa.applicant_user_id IS NOT NULL;
...
BEGIN
  ...
  SELECT applicant_user_id INTO prof_user_id
  FROM public.professional_applications
  WHERE id = consultation_record.professional_id;
