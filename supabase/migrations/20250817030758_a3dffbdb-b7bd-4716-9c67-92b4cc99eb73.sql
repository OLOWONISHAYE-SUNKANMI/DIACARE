...
BEGIN
  ...
  time_interval := schedule_time::INTERVAL;
  
  CASE freq
    WHEN 'daily' THEN
      base_date := date_trunc('day', current_time)::DATE;
      next_time := base_date::TIMESTAMP WITH TIME ZONE + time_interval;
      IF next_time <= current_time THEN
        next_time := next_time + INTERVAL '1 day';
      END IF;
    
    WHEN 'weekly' THEN
      base_date := date_trunc('week', current_time)::DATE;
      next_time := base_date::TIMESTAMP WITH TIME ZONE + 
                  (CASE schedule_day
    ...
    WHEN 'monthly' THEN
      base_date := date_trunc('month', current_time)::DATE;
      next_time := base_date::TIMESTAMP WITH TIME ZONE + time_interval;
      IF next_time <= current_time THEN
