...
CREATE TABLE public.scheduled_messages (
  ...
  schedule_time TIME,
  schedule_day TEXT, -- For weekly: 'monday', 'tuesday', etc.
  schedule_date DATE, -- For specific date scheduling
...
CREATE TABLE public.automated_messages (
  ...
  user_id UUID REFERENCES auth.users(id), -- For celebration messages
...
BEGIN
  CASE freq
    ...
    WHEN 'monthly' THEN
      next_time := date_trunc('month', current_time) + schedule_time;
      IF next_time <= current_time THEN
...
INSERT INTO public.message_templates (type, content, reactions) VALUES
('daily_motivation', '💪 Chaque contrôle glycémique est une victoire !', ARRAY['💪', '❤️', '👏']),
('daily_motivation', '🌅 Nouveau jour, nouvelles opportunités de bien gérer votre diabète', ARRAY['🌅', '💪', '❤️']),
('daily_motivation', '🎯 Votre persévérance d''aujourd''hui = votre santé de demain', ARRAY['🎯', '💪', '🏆']),
('daily_motivation', '👥 Ensemble, nous sommes plus forts face au diabète', ARRAY['👥', '❤️', '🤝']),
('daily_motivation', '🏆 Chaque petit geste compte dans votre parcours DARE', ARRAY['🏆', '💪', '👏']),
...
('educational', '🥗 Astuce nutrition : Les légumes verts à feuilles sont vos alliés pour stabiliser la glycémie.', ARRAY['🥗', '💡', '👍'], true),
('educational', '🏃‍♂️ Le saviez-vous ? 30 minutes de marche après un repas aident à réguler la glycémie.', ARRAY['🚶‍♂️', '💡', '👍'], true);
