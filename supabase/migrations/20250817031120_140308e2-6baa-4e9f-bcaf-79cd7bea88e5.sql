...
BEGIN
  CASE freq
    WHEN 'daily' THEN
      base_timestamp := date_trunc('day', current_time::TIMESTAMP WITH TIME ZONE);
      next_time := base_timestamp + schedule_time::INTERVAL;
    ...
    WHEN 'weekly' THEN
      ...
      base_timestamp := date_trunc('week', current_time::TIMESTAMP WITH TIME ZONE);
      next_time := base_timestamp + 
                  (CASE schedule_day
    ...
    WHEN 'monthly' THEN
      base_timestamp := date_trunc('month', current_time::TIMESTAMP WITH TIME ZONE);
      next_time := base_timestamp + schedule_time::INTERVAL;
      IF next_time <= current_time THEN
