...
const BottomNavigation = ({ activeTab, onTabChange }: BottomNavigationProps) => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 w-full bg-card/95 backdrop-blur-sm border-t border-border pb-safe-area-inset-bottom overflow-hidden z-[9999] shadow-lg">
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                style={{ minWidth: '44px' }}
              >
                <Icon className={`w-4 h-4 sm:w-5 sm:h-5 mb-0.5 sm:mb-1 flex-shrink-0 ${isActive ? "scale-110" : ""}`} />
                <span className="text-[10px] sm:text-xs font-medium leading-tight truncate max-w-full text-center">{tab.label}</span>
              </button>
            );
          })}
        </div>
