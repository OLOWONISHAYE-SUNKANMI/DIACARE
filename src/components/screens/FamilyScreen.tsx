...
//             >
//               {t('familyScreen.familySharingCode.button')}
//             </Button>
//           </div>
//         </CardContent>
//       </Card>
...
const FamilyScreen = (props: FamilyScreenProps) => {
  ...
  const handleGenerateCode = async () => {
    setLoading(true);
    const code = await generateAccessCode(email || undefined);
  ...
  return (
    <div className="flex-1 p-4 space-y-6 pb-24 animate-fade-in">
      ...
      <Card className="bg-yellow-50 border-yellow-200">
        ...
        <CardContent>
          <div className="text-center space-y-3">
            <div className="text-3xl font-mono font-bold text-yellow-800 bg-yellow-100 rounded-lg p-4">
              {generatedCode || '—'}{' '}
              {/* show empty placeholder if no code yet */}
            </div>
            <p className="text-sm text-yellow-700">
              {t('familyScreen.familySharingCode.subtitle')}
            </p>
              ...
              onClick={() => {
                if (!generatedCode) {
                  ...
                  toast({
                    title: 'Copied!',
                    description:
                      'The access code has been copied to your clipboard.',
                  });
                }
              }}
              variant="outline"
            ...
            >
              {!generatedCode
                ? t('familyScreen.familySharingCode.generateButton') // "Generate Invite"
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Generate Access Code</ModalHeader>
          <ModalCloseButton />
            ...
            <Button
              onClick={async () => {
                ...
                toast({
                  ...
                  description: 'You can now share this code with your family.',
                });
              }}
              // isLoading={loading}
            ...
            <div key={member.id}>
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  {!member.isPending && (
                    <div className="text-xs text-muted-foreground">
                      {t('familyScreen.familyMembers.lastSeenPlaceholder')}
                    </div>
                  )}
                </div>

                <Button size="sm" variant="outline" disabled={member.isPending}>
                  <Settings className="w-4 h-4" />
                </Button>
              </div>
              {index < familyMembers.length - 1 && (
